...
export function ProjectDetailsTab({ project, formatDate }: ProjectDetailsTabProps) {
  ...
  return (
    <TabsContent value="details" className="space-y-4 p-6">
      <div className="space-y-4">
        {renderBasicInfo()}
        {(project.budget || project.contractor || project.sourceOfFunds) && renderFinancialInfo()}
        {project.images && project.images.length > 0 && renderImages()}
        {project.progressUpdates && project.progressUpdates.length > 0 && renderProgressUpdates()}
      </div>
